...
fn eval_and_env<W: Into<Option<MockWord>>>(strict: bool, param: &MockParam, word: W)
...
fn eval_expect_assig<W, S>(strict: bool, param: &MockParam, word: W, expected_param_val: S)
    -> Result<Fields<String>, MockErr>
    where W: Into<Option<MockWord>>,
...
fn should_evaluate_appropriately() {
    ...
    assert_eq!(eval_expect_assig(false, &param, mock_word.clone(), val), Ok(word_fields.clone()));
    assert_eq!(eval_expect_assig(true, &param, mock_word.clone(), val), Ok(word_fields.clone()));
    ...
    assert_eq!(eval(false, &param, must_not_run.clone()), Err(bad_assig.clone()));
    assert_eq!(eval(true, &param, must_not_run.clone()), Err(bad_assig.clone()));
    ...
    assert_eq!(eval_expect_assig(false, &param, must_not_run.clone(), None), Ok(param_fields.clone()));
    assert_eq!(eval_expect_assig(true, &param, must_not_run.clone(), None), Ok(param_fields.clone()));
    assert_eq!(eval_expect_assig(false, &param, None, None), Ok(param_fields.clone()));
    assert_eq!(eval_expect_assig(true, &param, None, None), Ok(param_fields.clone()));
    ...
    assert_eq!(eval_expect_assig(false, &param, must_not_run.clone(), None), Ok(Fields::Zero));
    assert_eq!(eval_expect_assig(true, &param, mock_word.clone(), val), Ok(word_fields.clone()));
    assert_eq!(eval_expect_assig(false, &param, None, None), Ok(Fields::Zero));
    assert_eq!(eval_expect_assig(true, &param, None, ""), Ok(Fields::Zero));
...
fn should_propagate_errors_from_word_if_applicable() {
    ...
    let bad_assig = MockErr::ExpansionError(ExpansionError::BadAssig(param.to_string()));
    eval(false, &param, must_not_run.clone()).unwrap();
    assert_eq!(eval(true, &param, mock_word_error(false)), Err(bad_assig.clone()));
    eval(false, &param, None).unwrap();
    assert_eq!(eval(true, &param, None), Err(bad_assig.clone()));
