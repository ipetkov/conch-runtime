...
use conch_runtime::new_eval::{Fields, remove_largest_suffix};
...
{
    let mut env = ();
    remove_largest_suffix(param, word.into(), &mut env)
        .pin_env(env)
        .wait()
}
...
fn should_propagate_cancel_if_required() {
    ...
    // Param not present
    let param = MockParam::Fields(None);
    test_cancel!(remove_largest_suffix(&param, must_not_run.clone(), env));
    test_cancel!(remove_largest_suffix::<_, MockWord, _>(&param, None, env));

    // Present and non-empty
    let param = MockParam::Fields(Some(Fields::Single("foo".to_owned())));
    test_cancel!(remove_largest_suffix(&param, must_cancel.clone(), env));
    test_cancel!(remove_largest_suffix::<_, MockWord, _>(&param, None, env));
