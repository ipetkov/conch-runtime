...
use conch_runtime::new_eval::{Fields, remove_smallest_prefix};
...
fn should_evaluate_appropriately() {
    ...
    assert_eq!(eval(&param, mock_word.clone()), Ok(Fields::Single(" \u{1F4A9} d abced".to_owned())));
    assert_eq!(eval(&param, mock_word_wild), Ok(Fields::Single(s.to_owned())));
    assert_eq!(eval(&param, mock_word_split), Ok(Fields::Single(" d abced".to_owned())));
    assert_eq!(eval(&param, None), Ok(Fields::Single(s.clone())));
    let param = MockParam::Fields(Some(Fields::Single("abc".to_owned())));
    assert_eq!(eval(&param, mock_word.clone()), Ok(Fields::Single("".to_owned())));
...
fn should_propagate_cancel_if_required() {
    ...
    // Param not present
    let param = MockParam::Fields(None);
    test_cancel!(remove_smallest_prefix(&param, must_not_run.clone(), env));
    test_cancel!(remove_smallest_prefix::<_, MockWord, _>(&param, None, env));

    // Present and non-empty
    let param = MockParam::Fields(Some(Fields::Single("foo".to_owned())));
    test_cancel!(remove_smallest_prefix(&param, must_cancel.clone(), env));
    test_cancel!(remove_smallest_prefix::<_, MockWord, _>(&param, None, env));
