...
{
    let mut env = ();
    remove_smallest_suffix(param, word.into(), &mut env)
        .pin_env(env)
        .wait()
}
...
fn should_evaluate_appropriately() {
    ...
    assert_eq!(eval(&param, mock_word.clone()), Ok(Fields::Single("abc\u{1F4A9} d ".to_owned())));
    assert_eq!(eval(&param, mock_word_wild), Ok(Fields::Single(s.clone())));
    assert_eq!(eval(&param, mock_word_split), Ok(Fields::Single("abc\u{1F4A9} ".to_owned())));
