...
use conch_runtime::eval::{remove_largest_prefix, Fields};
...
async fn eval<W: Into<Option<MockWord>>>(
    ...
    let mut env = ();
    remove_largest_prefix(param, word.into(), &mut env).await
}
...
async fn should_evaluate_appropriately() {
    ...
    assert_eq!(
        ...
        Ok(Fields::Single("dabcde".to_owned()))
    );
