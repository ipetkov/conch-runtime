...
use conch_runtime::env::{
    ArgsEnv, ArgumentsEnvironment, Env, EnvConfig, LastStatusEnvironment, VariableEnvironment,
};
...
async fn test_eval_parameter_with_set_vars() {
    ...
    assert_eq!(
        ...
        Some(Fields::Single(getpid().to_string()))
    );
    ...
    assert_eq!(
        Question.eval(false, &env),
        Some(Fields::Single("0".to_owned()))
    );
    env.set_last_status(ExitStatus::Code(3));
    assert_eq!(
        Question.eval(false, &env),
        Some(Fields::Single("3".to_owned()))
    );
    // Signals should have 128 added to them
    env.set_last_status(ExitStatus::Signal(5));
    assert_eq!(
        Question.eval(false, &env),
        Some(Fields::Single("133".to_owned()))
    );

    assert_eq!(
        Positional(0).eval(false, &env),
        Some(Fields::Single(env.name().clone()))
    );
    ...
    assert_eq!(
        ...
        Some(Fields::Single(var1))
    );
    assert_eq!(
        Var("var2".to_owned()).eval(false, &env),
        Some(Fields::Single(var2))
    );
    assert_eq!(
        Var("var3".to_owned()).eval(false, &env),
        Some(Fields::Single(var3))
    );

    assert_eq!(
        Pound.eval(false, &env),
        Some(Fields::Single("3".to_owned()))
    );
}
...
async fn test_eval_parameter_splitting_with_default_ifs() {
    ...
    assert_eq!(
        ...
        Some(Fields::Single(val1.clone()))
    );
    assert_eq!(
        Positional(2).eval(false, &env),
        Some(Fields::Single(val2.clone()))
    );

    assert_eq!(
        Var("var1".to_owned()).eval(false, &env),
        Some(Fields::Single(val1))
    );
    assert_eq!(
        Var("var2".to_owned()).eval(false, &env),
        Some(Fields::Single(val2))
    );
}
...
async fn test_eval_parameter_splitting_with_custom_ifs() {
    ...
    assert_eq!(Positional(1).eval(true, &env), Some(fields_foo_bar.clone()));
    assert_eq!(
        Positional(2).eval(true, &env),
        Some(fields_all_blanks.clone())
    );
    ...
    assert_eq!(
        Var("var2".to_owned()).eval(true, &env),
        Some(fields_all_blanks)
    );
    ...
    assert_eq!(
        Question.eval(true, &env),
        Some(Fields::Single("".to_owned()))
    );
    ...
    assert_eq!(
        Positional(3).eval(false, &env),
        Some(Fields::Single(val3.clone()))
    );
